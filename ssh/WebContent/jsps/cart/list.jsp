...
<script type="text/javascript">
						...
						function() {
							...
							if (quantity == 1) {
								if (confirm("您是否真要删除该条目？")) {
									location = "${pageContext.request.contextPath}/CartItem/deleteCart.do?cartItemId="
											+ id;
								}
							} else {
	...
	function sendUpdateQuantity(id, quantity) {
		...
		$.ajax({
			...
			url : "${pageContext.request.contextPath}/CartItem/updateQuantity.do",
			data : {
			...
			success : function(result) {
				//1. 修改数量
				$("#" + id + "Quantity").val(result.quantitys);
				//2. 修改小计
				$("#" + id + "Subtotal").text(result.subtotals);
				alert(result.subtotals);
				//3. 重新计算总计
				showTotal();
			}
		});
	...
	<c:choose>
		...
		<c:otherwise>
					...
					<tr align="center">
						...
						<td align="left" width="70px"><a class="linkImage"
							href="<c:url value='/jsps/book/desc.jsp'/>"><img border="0"
								width="54" align="top"
								src="<c:url value='/${cartItem.bookImageB }'/>" /></a></td>
						<td align="left" width="400px"><a
							href="<c:url value='/jsps/book/desc.jsp'/>"><span>${cartItem.bookName }</span></a>
						</td>
						<td><span>&yen;<span class="currPrice">${cartItem.bookCurrPrice }</span></span></td>
						<td><a class="jian" id="${cartItem.cartitemId }Jian">
						</a><input
							class="quantity" readonly="readonly"
							id="${cartItem.cartitemId}Quantity" type="text"
							value="${cartItem.cartitemCount}" />
							
							<a class="jia"
							id="${cartItem.cartitemId }Jia"></a></td>
						<td width="100px"><span class="price_n">&yen;<span
								class="subTotal" id="${cartItem.cartitemId }Subtotal">${cartItem.bookCurrPrice }</span></span>
						</td>
						<td><a
							href="${pageContext.request.contextPath}/CartItem/deleteCart.do?cartItemId=${cartItem.cartitemId }">删除</a>
						</td>
					</tr>
				</c:forEach>
			...
			<form id="jieSuanForm" action="${pageContext.request.contextPath}/CartItem/loadCartItems.do"
				method="post">
				<input type="hidden" name="cartItemId" id="cartItemIds" /> <input
					type="hidden" name="total" id="hiddenTotal" />
			</form>
